...
def run_segmentation(image_bytes: bytes, prompt: str):
    ...
    try:
        masks, _scores, _ = sam_predictor.predict(
            point_coords=None,
            point_labels=None,
            box=input_boxes[0].cpu().numpy(),
            multimask_output=False,
        )
        if masks is not None and len(masks) > 0:
            binary_mask = (masks[0] > 0).astype(np.uint8) * 255
            print(f"Mask generated via box prompt, shape: {binary_mask.shape}")
    except Exception as e:
        print(f"Box-prompt SAM failed: {e}")
...
def segment():
    if not load_models():
        # Still return 200 with success=False so the UI can surface the reason
        # without a hard HTTP error, matching the error-reporting contract used
        # for validation failures below.
        pass
